...
RDEPENDS_${PN} += "xrt libcurl"

SRC_URI = "file://app \
           file://conf.json \
           file://cred.json \
           file://libaccelize_drm.so.2.5.3 \
...
do_install() {
        ...
        install -m 0755 ${S}/libaccelize_drm.so.2.5.3 ${D}${libdir}
        install -m 0755 ${S}/libjsoncpp.so.1.9.4 ${D}${libdir}
        ln -rs ${D}${libdir}/libaccelize_drm.so.2.5.3 ${D}${libdir}/libaccelize_drm.so.2
