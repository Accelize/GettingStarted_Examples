...
FROM xilinx/xilinx_runtime_base:alveo-2022-1-ubuntu-1804 AS builder
...
FROM xilinx/xilinx_runtime_base:alveo-2022-1-ubuntu-1804
ENV LD_LIBRARY_PATH /opt/xilinx/xrt/lib
...
COPY xclbin/*.xclbin /app.xclbin
COPY app/conf.json /conf.json
COPY app/dna_reader_cred.json /cred.json
