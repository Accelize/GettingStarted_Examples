 ...
 *
 */
int metering_test(uint32_t mode, int slot_id, int pf_id, int bar_id)
{
  int ret=0;
  uint32_t reg=0;
  uint32_t drm_base_addr = 0x10000;
  uint32_t user_ip_2     = 0x20000;
  uint32_t user_ip_1     = 0x00000;
   ...
   while(c != 'q') {
       ...
       if(c=='d') {  
        ...
        if(c=='a') {
			...
			my_read_drm(pci_bar_handle, user_ip_1+0, &reg1);
			my_read_drm(pci_bar_handle, user_ip_1+4, &reg2);
			my_read_drm(pci_bar_handle, user_ip_1+8, &reg3);
			my_read_drm(pci_bar_handle, user_ip_1+12, &reg4);
			printf("%s: UI1: Addr=0x%X, Val=0x%x => act_code_rdy=%d, activated=%d\n", __FUNCTION__, user_ip_1+0, reg1, (reg1&0x02)>>1, (reg1&0x01));
			printf("\t=> Index=%d, act_code_rdy=%d, activated=%d\n", (reg1&0xFC)>>2, (reg1&0x02)>>1, (reg1&0x01));
			printf("%s: UI1: Addr=0x%X, Val=0x%x\n", __FUNCTION__, user_ip_1+4, reg2);		
			printf("%s: UI1: Addr=0x%X, Val=0x%x\n", __FUNCTION__, user_ip_1+8, reg3);
			printf("%s: UI1: Addr=0x%X, Val=0x%x\n", __FUNCTION__, user_ip_1+12, reg4);
			
			my_read_drm(pci_bar_handle, user_ip_2+0, &reg1);
			my_read_drm(pci_bar_handle, user_ip_2+4, &reg2);
			my_read_drm(pci_bar_handle, user_ip_2+8, &reg3);
			my_read_drm(pci_bar_handle, user_ip_2+12, &reg4);
			printf("%s: UI2: Addr=0x%X, Val=0x%x => act_code_rdy=%d, activated=%d\n", __FUNCTION__, user_ip_2+0, reg1, (reg1&0x02)>>1, (reg1&0x01));
			printf("\t=> Index=%d, act_code_rdy=%d, activated=%d\n", (reg1&0xFC)>>2, (reg1&0x02)>>1, (reg1&0x01));
			printf("%s: UI2: Addr=0x%X, Val=0x%x\n", __FUNCTION__, user_ip_2+4, reg2);		
			printf("%s: UI2: Addr=0x%X, Val=0x%x\n", __FUNCTION__, user_ip_2+8, reg3);
			printf("%s: UI2: Addr=0x%X, Val=0x%x\n", __FUNCTION__, user_ip_2+12, reg4);
		}
 ...
 *
 */
int rw_test(uint32_t mode, int slot_id, int pf_id, int bar_id)
{
  int ret=0;
  uint32_t reg=0;
  uint32_t drm_base_addr = 0x10000;
  ...
  if(fpga_pci_attach(slot_id, pf_id, bar_id, 0, &pci_bar_handle)) {
   ...
   while(c != 'q') {
       ...
       if(c=='d') {  
		...
		if(c=='w') {
			my_write_drm(pci_bar_handle, user_ip_1+4, 0x1234ABCD);
			my_write_drm(pci_bar_handle, user_ip_2+4, 0xABCD1234);
		}
		
		if(c=='x') {
			my_write_drm(pci_bar_handle, user_ip_1+8, 0xB16B00B5);
			my_write_drm(pci_bar_handle, user_ip_2+8, 0xB00B5B16);
		}
		
		if(c=='c') {
			my_write_drm(pci_bar_handle, user_ip_1+12, 0xACCE717E);
			my_write_drm(pci_bar_handle, user_ip_2+12, 0x717EACCE);
		}
    }
    printf("%s: Application exited, press any key to continue\n", __FUNCTION__);
    c=getchar();
...
int main(int argc, char **argv) 
{
    ...
    if(rw_test(mode, 0, FPGA_APP_PF, APP_PF_BAR0)) {
	...
	/* Metering Test */
    printf("===== Metering test =====\n");
    if(metering_test(mode, 0, FPGA_APP_PF, APP_PF_BAR0)) {
        printf("Metering test failed");
        return 1;
    }
