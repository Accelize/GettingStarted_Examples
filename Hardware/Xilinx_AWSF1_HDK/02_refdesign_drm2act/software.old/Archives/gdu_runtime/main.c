...
int main(int argc, char **argv) {
    ...
    */
    ...
    rc = drm_dump(base_addr, slot_id, FPGA_APP_PF, APP_PF_BAR0);
    fail_on(rc, out, "Dump DRM Registers failed");
...
int drm_dump(uint32_t base_addr, int slot_id, int pf_id, int bar_id)
{
  ...
  for(addr=0; addr<=128; addr+=4) {
    rc = fpga_pci_peek(pci_bar_handle, (base_addr+addr), &value);
    fail_on(rc, out, "Unable to read read from the fpga !");
    printf("addr= 0x%x register: 0x%x\n", (base_addr+addr), value);
